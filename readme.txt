...
= 1.6158 =
* Fix: removed GitHub Plugin URI and Primary Branch headers from the main plugin file — these are development-side metadata for GitHub Updater tooling only and must not be present in a WordPress.org-distributed package.
* Fix: renamed ELC_Link_Checker class to TIMU_ELC_Link_Checker for prefix uniqueness compliance.
* Fix: registered a custom timu_elc_weekly cron schedule; wp_schedule_event no longer relies on the unregistered built-in weekly schedule.
* Fix: Tested up to corrected to 6.7 (the highest actually-tested released version).
* Fix: License URI header added to the main plugin file.
...
= 1.6158 =
WP.org compliance fixes: GitHub Plugin URI header removed, class prefix corrected to TIMU_ELC_Link_Checker, custom weekly cron schedule registered, Tested up to corrected to 6.7.
